...
- **Five institution forms live as Word templates in `Templates\Forms\`** — «فورم ۱ درخواست ایتام», «۲ درخواست نیازمندان», «۳ درخواست درمان», «۴ تحقیق و بررسی», «۷ پرونده بخش درمان» — rebuilt from the institution's own PDFs, A4, one page each, RTL, B Nazanin / B Titr, navy section bars. They must stay in the `Forms` subfolder (`ReportTemplateHelper.DiscoverCaseTemplates` reads only the `Templates` root).
...
- **Where the 3 h goes (measured 2026-09-06).** Summed test durations are only ~78 min, so ~⅔ of the wall clock is fixture/host overhead, not test bodies. One single test dominates: `SaveAssistance_NonCash_PersistsPackageIdZeroAmount_AndUpdatesCaseRequestType` takes **21 m 17 s** on its own (~11 % of the run) — it drives the real `FrmFinance` form; fix or shorten it before optimising anything else. Next heaviest are the backup / disaster-recovery / AI-performance classes at 16–40 s each.
