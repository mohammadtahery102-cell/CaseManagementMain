...
- **Run time scales with test *count*, not with a few slow tests, and `/Parallel` barely matters (four measurements, 2026-09-06).**
  ...
  |---|---|---|---|
  | full | 664 | `/Parallel` | **3 h 12 m** |
  | excluding backup/DR/perf/screenshot | 619 | none | **2 h 59 m** |
  | targeted (one feature area) | 113 | none | **5.3 m** |
  | sum of reported per-test durations | 664 | — | ~78 m |

  619 tests cost 34× the time of 113 tests for 5.5× the tests, and dropping `/Parallel` changed almost nothing. So the cost is **per-test fixture overhead** — every `[TestInitialize]` builds a temp SQLite DB and runs `EnsureDatabaseObjects()` (~0.9 s alone) — and it is not captured in reported per-test durations. Two earlier diagnoses recorded here were wrong and are retired: it is not a deadlock, and it is not `/Parallel` contention. `SaveAssistance_NonCash…` really does report 21 m under `/Parallel` versus 1 s targeted, but excluding it changed the total by minutes, so it is a symptom, not the cause.
- **Fast lane = an *inclusion* filter, never an exclusion filter.** Excluding the handful of obviously-slow classes removes only ~45 tests and still costs ~3 h — do not bother. Instead name the areas a change touches, e.g.
  `vstest.console.exe <dll> /TestCaseFilter:"FullyQualifiedName~Permission|FullyQualifiedName~Export|FullyQualifiedName~Duplicate"` — that shape ran 113 tests in 5.3 min. The full 3 h run belongs to pre-release verification only.
- **Real fix (not yet done):** share one initialised template database across tests instead of rebuilding the schema per test. That is where the ~2 h goes.
...
| 90 | **الزام ۱ نسخهٔ تحویلی — هشدارِ شباهت در لحظهٔ ذخیره** با متدِ تازهٔ `DuplicateDetector.FindSimilarCases` (آستانهٔ پیش‌فرض ۹۰) که در هر دو مسیرِ ثبت و ویرایشِ `FrmCase` صدا زده می‌شود؛ **موتورِ دومی ساخته نشد** | قاعدهٔ صریحِ پروژه: «DuplicateDetector تنها موتورِ تشخیصِ تکراری است». `Detect()` گزارشِ دسته‌ایِ همه‌با‌همه است و برای دکمهٔ ذخیره سنگین؛ متدِ تازه فقط *یک* رکورد را می‌سنجد ولی از همان `Normalize`/`Similarity` استفاده می‌کند، پس دو تعریفِ متفاوت از «شباهت» هرگز پیدا نمی‌شود. هشدار **نرم** است نه مسدودکننده (هم‌نامِ واقعی در این جامعه فراوان است) و تأییدِ کاربر در حسابرسی ثبت می‌شود. جفتِ هم‌خانوار کنار گذاشته می‌شود، وگرنه هر خانوارِ چندپرونده‌ای سیلی از هشدارِ نادرست می‌ساخت. ۶ آزمون در `SaveTimeSimilarityTests`. | 2026-09-06 | active |
| 91 | **الزام ۷ — فیلدِ «سایت»** به‌صورت ستونِ مسطحِ `TblCase.Site TEXT NULL` + `IX_TblCase_Site`، نه جدولِ مرجع و نه ستونِ جدولِ ماژول | برای هر شش نوعِ پرونده معنی دارد، پس همان استدلالِ تصمیم #۱۹ (ستونِ مسطح روی `TblCase`) صادق است. متنِ آزاد و نه کلیدِ خارجی، چون فهرستِ سایت‌ها هنوز تثبیت نشده و جدولِ مرجعِ خالی فقط یک دروازهٔ اضافی سرِ راهِ کاربر می‌گذاشت. **کارت شناسایی و RDLC عمداً دست‌نخورده ماندند**: افزودن به `CardFieldCatalog` بدونِ عنصرِ متناظر در قالب، فیلدی می‌سازد که تنظیمش بی‌صدا نادیده گرفته می‌شود (تصمیم #۴۸)، و افزودنِ فیلد به RDLC پرریسک‌ترین تغییرِ گزارشیِ موجود است (تصمیم #۸۱). ۵ آزمون در `SiteFieldTests`. | 2026-09-06 | active — درجِ `{{Site}}` در سه قالبِ Word و در قالبِ کارت، کارِ ویرایشِ فایل است نه کد |
| 92 | **الزام ۱۲ — بازکردنِ مستقیمِ پرونده از نتایجِ جستجو** با دابل‌کلیک/Enter در `FrmAdvancedSearch`، و اجرای دوبارهٔ جستجو پس از بسته‌شدنِ فرم | کارت‌های نواقصِ داشبورد از قبل به این فرم می‌آمدند ولی آنجا بن‌بست بود. سازندهٔ `FrmCase(int)` از قبل وجود داشت و **هیچ فرمی از آن استفاده نمی‌کرد** — فقط سیم‌کشی کم بود. اجرای دوبارهٔ جستجو لازم است وگرنه کاربر نمی‌فهمد نقصی که همان لحظه رفع کرد اثر کرده یا نه. | 2026-09-06 | active |
| 93 | **الزامات ۵/۱۱/۱۲/۱۳ — نقطهٔ فراخوانِ بازمحاسبهٔ گروهی** به‌صورت دکمهٔ «بازمحاسبهٔ تکمیل و امتیاز» در تبِ نگهداریِ `FrmSettings`؛ ترتیب عمدی است: اول تکمیل، بعد امتیاز | `CaseCompletionService.RecalculateAll` و `VulnerabilityScoreService.RecalculateAll` از فاز ۵ نوشته شده بودند ولی **هیچ فراخوان‌کننده‌ای نداشتند** (تصمیم #۱۷ آن را به «صفحهٔ ادمینِ آینده» موکول کرده بود). نتیجه روی دادهٔ واقعی: هر ۱٬۶۶۱ پرونده `CompletionStatusCode` و `VulnerabilityBand` را NULL داشتند، پس کارت‌های کیفیت/ریسکِ داشبورد، فیلترهای جستجو و بلوکِ وضعیتِ گزارشِ تفصیلی برای کلِ جمعیت خالی بودند — کد بی‌عیب بود و فقط اجرا نمی‌شد. «اول تکمیل بعد امتیاز» چون `CompletionStatus` خودش یکی از حقایقِ امتیازدهی است. اندازه‌گیری روی دادهٔ واقعی: ۱٬۶۶۱ پرونده در ۶۴ ثانیه، NULL از ۱٬۶۶۱ به ۰. | 2026-09-06 | active |
| 94 | **الزام ۱۳ — منبعِ گزارشِ `CompletionStatus` («وضعیت تکمیل و اسناد»)** با ۱۶ ستون، همه از ستون‌های کش‌شده و زیرپرس‌وجوهای اسکالر | همهٔ منابعِ گزارشیِ موجود ردیف‌محورند؛ پرسشِ مدیریتی «چند درصدِ پرونده‌های هر سایت/ولایت/نوع اسنادشان کامل است» با گروه‌بندیِ همین منبع پاسخ می‌گیرد. **زیرپرس‌وجوی اسکالر و نه JOIN** — جوین ردیفِ پرونده را تکثیر و هر شمارش و گروه‌بندی را خراب می‌کرد (همان درسِ تصمیم #۵۰). شمارِ «دستهٔ الزامیِ کم» عیناً همان تعریفِ دروازهٔ فعال‌سازی است، از جمله قاعدهٔ شرطیِ «عکس کارت معلولیت» (تصمیم #۵۳)، تا گزارش و دروازه دربارهٔ یک پرونده حرفِ متفاوت نزنند. ⚠ بدونِ اجرای بازمحاسبهٔ #۹۳ برای پرونده‌های قدیمی خالی درمی‌آید. | 2026-09-06 | active |
| 95 | **الزام ۱۰ — «عکس محل معلولیت» در خروجی Word** با `{{DisabilityAreaPhoto}}`؛ مسیر از **جدولِ اسناد** خوانده می‌شود (جدیدترین سندِ دستهٔ `DISABILITY_AREA_PHOTO`)، نه از ستونی تازه روی `TblCase` | عکس از قبل قابلِ بارگذاری بود ولی فقط به‌عنوانِ سند (تصمیم #۱۱) و هیچ خروجی‌ای رندرش نمی‌کرد — از دیدِ کاربر یعنی «کار نمی‌کند». افزودنِ ستونِ عکس روی `TblCase` یعنی دو مسیرِ ذخیره برای یک واقعیت و همان واگراییِ دوگانه‌ای که این پروژه بارها بهایش را داده. پروندهٔ بدونِ این سند رشتهٔ خالی می‌گیرد و `RemoveUnusedPlaceholdersEverywhere` جای خالی را پاک می‌کند، پس خروجیِ پنج نوعِ دیگرِ پرونده دست‌نخورده می‌ماند. RDLC عمداً لمس نشد (تصمیم #۸۱). | 2026-09-06 | active — درجِ `{{DisabilityAreaPhoto}}` در قالب‌ها کارِ ویرایشِ فایل است |
